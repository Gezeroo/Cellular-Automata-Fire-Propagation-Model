...
num_experimentos = 1  # Ajuste conforme necessário
...
for exp_num, nome_exp in enumerate(nome_exps):
    imagens = []

    for nome_img in nomes_imgs:
        nome_arquivo = f"exp{nome_exp}_{nome_img}"
    ...
    if len(imagens) != len(nomes_imgs):
        print(f"[ERRO] Experimento {nome_exp} está incompleto. Pulando.")
        continue
    ...
    output_path = os.path.join(output_dir, f"experimento_{nome_exp}.png")
